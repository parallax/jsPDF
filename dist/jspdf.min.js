...
Copyright (c) 2012 2012 Willow Systems Corporation, willow-systems.com
 jsPDF ( 2013-01-07T00:31 commit ID e07f98f849424b231a0fd3f90bdfb1977ce12f9b )
...
MIT license.
*/
var jsPDF=function(){function z(g,e,d){g="undefined"===typeof g?"p":g.toString().toLowerCase();"undefined"===typeof e&&(e="mm");"undefined"===typeof d&&(d="a4");var c=d.toString().toLowerCase(),a=[],f=0,b={a3:[841.89,1190.55],a4:[595.28,841.89],a5:[420.94,595.28],letter:[612,792],legal:[612,1008]},l="0 g",s=0,h=[],m=2,x=!1,u=[],t={},D={},H=16,w,p,A,k,C={title:"",subject:"",author:"",keywords:"",creator:""},N=0,O=0,y={},F=new v(y);if("pt"==e)k=1;else if("mm"==e)k=72/25.4;else if("cm"==e)k=72/2.54;
else if("in"==e)k=72;else throw"Invalid unit: "+e;if(c in b)p=b[c][1]/k,A=b[c][0]/k;else try{p=d[1],A=d[0]}catch(T){throw"Invalid format: "+d;}if("p"===g||"portrait"===g)g="p",A>p&&(g=A,A=p,p=g);else if("l"===g||"landscape"===g)g="l",p>A&&(g=A,A=p,p=g);else throw"Invalid orientation: "+g;var q=function(B){return B.toFixed(2)},J=function(B){var b=B.toFixed(0);return 10>B?"0"+b:b},j=function(B){x?h[s].push(B):(a.push(B),f+=B.length+1)},I=function(){m++;u[m]=f;j(m+" 0 obj");return m},Q=function(B){j("stream");
j(B);j("endstream")},R=function(){s++;x=!0;h[s]=[];j(q(0.200025*k)+" w");j("0 G");0!==N&&j(N.toString(10)+" J");0!==O&&j(O.toString(10)+" j");F.publish("addPage",{pageNumber:s})},P=function(B,b){var f;void 0===B&&(B=t[w].fontName);void 0===b&&(b=t[w].fontStyle);try{f=D[B][b]}catch(d){f=void 0}if(!f)throw Error("Unable to look up font label for font '"+B+"', '"+b+"'. Refer to getFontList() for available fonts.");return f},K=function(){x=!1;a=[];u=[];j("%PDF-1.3");var b=A*k,d=p*k,c,e;for(c=1;c<=s;c++)I(),
j("<</Type /Page"),j("/Parent 1 0 R"),j("/Resources 2 0 R"),j("/Contents "+(m+1)+" 0 R>>"),j("endobj"),e=h[c].join("\n"),I(),j("<</Length "+e.length+">>"),Q(e),j("endobj");u[1]=f;j("1 0 obj");j("<</Type /Pages");c="/Kids [";for(e=0;e<s;e++)c+=3+2*e+" 0 R ";j(c+"]");j("/Count "+s);j("/MediaBox [0 0 "+q(b)+" "+q(d)+"]");j(">>");j("endobj");for(var l in t)t.hasOwnProperty(l)&&(b=t[l],b.objectNumber=I(),j("<</BaseFont/"+b.PostScriptName+"/Type/Font"),"string"===typeof b.encoding&&j("/Encoding/"+b.encoding),
...
G(C.keywords)+")");C.creator&&j("/Creator ("+G(C.creator)+")");l=new Date;j("/CreationDate (D:"+[l.getFullYear(),J(l.getMonth()+1),J(l.getDate()),J(l.getHours()),J(l.getMinutes()),J(l.getSeconds())].join("")+")");j(">>");j("endobj");I();j("<<");j("/Type /Catalog");j("/Pages 1 0 R");j("/OpenAction [3 0 R /FitH null]");j("/PageLayout /OneColumn");j(">>");j("endobj");l=f;j("xref");j("0 "+(m+1));j("0000000000 65535 f ");for(g=1;g<=m;g++)b=u[g].toFixed(0),b=10>b.length?Array(11-b.length).join("0")+b:b,
...
!1;l=[];a=0;for(h=c.length;a<h;a++)(s=e[c.charCodeAt(a)])?l.push(String.fromCharCode(s)):l.push(c[a]),l[a].charCodeAt(0)>>8&&(g=!0);c=l.join("")}for(a=c.length;void 0===g&&0!==a;)c.charCodeAt(a-1)>>8&&(g=!0),a--;if(g){l=d.noBOM?[]:[254,255];a=0;for(h=c.length;a<h;a++){s=c.charCodeAt(a);d=s>>8;if(d>>8)throw Error("Character at position "+a.toString(10)+" of string '"+c+"' exceeds 16bits. Cannot be encoded into UCS-2 BE");l.push(d);l.push(s-(d<<8))}c=String.fromCharCode.apply(void 0,l)}return c.replace(/\\/g,
...
events:F,scaleFactor:k,pageSize:{width:A,height:p}};y.addPage=function(){R();return this};y.text=function(b,a,c,f){var d,h;"number"===typeof b&&(d=b,h=a,b=c,a=d,c=h);"string"===typeof b&&b.match(/[\n\r]/)&&(b=b.split(/\r\n|\r|\n/g));"undefined"===typeof f?f={noBOM:!0,autoencode:!0}:(void 0===f.noBOM&&(f.noBOM=!0),void 0===f.autoencode&&(f.autoencode=!0));if("string"===typeof b)f=G(b,f);else if(b instanceof Array){b=b.concat();for(d=b.length-1;-1!==d;d--)b[d]=G(b[d],f);f=b.join(") Tj\nT* (")}else throw Error('Type of text must be string or Array. "'+
b+'" is not recognized.');j("BT\n/"+w+" "+H+" Tf\n"+H+" TL\n"+l+"\n"+q(a*k)+" "+q((p-c)*k)+" Td\n("+f+") Tj\nET");return this};y.line=function(b,f,a,c){j(q(b*k)+" "+q((p-f)*k)+" m "+q(a*k)+" "+q((p-c)*k)+" l S");return this};y.lines=function(b,f,a,c,d){var h,e;"number"===typeof b&&(h=b,e=f,b=a,f=h,a=e);d=L(d);c=void 0===c?[1,1]:c;j((f*k).toFixed(3)+" "+((p-a)*k).toFixed(3)+" m ");h=c[0];c=c[1];e=0;for(var l=b.length,g,s,m,q,x=a;e<l;e++)a=b[e],2===a.length?(f=a[0]*h+f,x=a[1]*c+x,j((f*k).toFixed(3)+
" "+((p-x)*k).toFixed(3)+" l")):(g=a[0]*h+f,s=a[1]*c+x,m=a[2]*h+f,q=a[3]*c+x,f=a[4]*h+f,x=a[5]*c+x,j((g*k).toFixed(3)+" "+((p-s)*k).toFixed(3)+" "+(m*k).toFixed(3)+" "+((p-q)*k).toFixed(3)+" "+(f*k).toFixed(3)+" "+((p-x)*k).toFixed(3)+" c"));j(d);return this};y.rect=function(b,a,f,c,d){d=L(d);j([q(b*k),q((p-a)*k),q(f*k),q(-c*k),"re",d].join(" "));return this};y.triangle=function(b,a,f,c,d,h,e){this.lines([[f-b,c-a],[d-f,h-c],[b-d,a-h]],b,a,[1,1],e);return this};y.roundedRect=function(b,a,f,c,d,h,
e){var l=4/3*(Math.SQRT2-1);this.lines([[f-2*d,0],[d*l,0,d,h-h*l,d,h],[0,c-2*h],[0,h*l,-(d*l),h,-d,h],[-f+2*d,0],[-(d*l),0,-d,-(h*l),-d,-h],[0,-c+2*h],[0,-(h*l),d*l,-h,d,-h]],b+d,a,[1,1],e);return this};y.ellipse=function(b,a,f,c,d){d=L(d);var h=4/3*(Math.SQRT2-1)*f,e=4/3*(Math.SQRT2-1)*c;j([q((b+f)*k),q((p-a)*k),"m",q((b+f)*k),q((p-(a-e))*k),q((b+h)*k),q((p-(a-c))*k),q(b*k),q((p-(a-c))*k),"c"].join(" "));j([q((b-h)*k),q((p-(a-c))*k),q((b-f)*k),q((p-(a-e))*k),q((b-f)*k),q((p-a)*k),"c"].join(" "));
j([q((b-f)*k),q((p-(a+e))*k),q((b-h)*k),q((p-(a+c))*k),q(b*k),q((p-(a+c))*k),"c"].join(" "));j([q((b+h)*k),q((p-(a+c))*k),q((b+f)*k),q((p-(a+e))*k),q((b+f)*k),q((p-a)*k),"c",d].join(" "));return this};y.circle=function(b,a,f,c){return this.ellipse(b,a,f,f,c)};y.setProperties=function(b){for(var a in C)C.hasOwnProperty(a)&&b[a]&&(C[a]=b[a]);return this};y.setFontSize=function(b){H=b;return this};y.setFont=function(b,a){w=P(b,a);return this};y.setFontStyle=y.setFontType=function(b){w=P(void 0,b);return this};
...
{"0":0,butt:0,but:0,bevel:0,1:1,round:1,rounded:1,circle:1,2:2,projecting:2,project:2,square:2,milter:2};y.setLineCap=function(b){var a=this.CapJoinStyles[b];if(void 0===a)throw Error("Line cap style of '"+b+"' is not recognized. See or extend .CapJoinStyles property for valid styles");N=a;j(a.toString(10)+" J");return this};y.setLineJoin=function(b){var a=this.CapJoinStyles[b];if(void 0===a)throw Error("Line join style of '"+b+"' is not recognized. See or extend .CapJoinStyles property for valid styles");
O=a;j(a.toString(10)+" j");return this};y.output=function(b,a){switch(b){case void 0:return K();case "save":for(var f=new BlobBuilder,c=K(),d=c.length,h=new Uint8Array(new ArrayBuffer(d)),e=0;e<d;e++)h[e]=c.charCodeAt(e);f.append(h);f=f.getBlob("application/pdf");saveAs(f,a);break;case "datauristring":case "dataurlstring":return"data:application/pdf;base64,"+btoa(K());case "datauri":case "dataurl":document.location.href="data:application/pdf;base64,"+btoa(K());break;default:throw Error('Output type "'+
b+'" is not supported.');}};y.save=function(b){y.output("save",b)};for(var E in z.API)if(z.API.hasOwnProperty(E))if("events"===E&&z.API.events.length){g=F;e=z.API.events;c=d=void 0;for(b=e.length-1;-1!==b;b--)d=e[b][0],c=e[b][1],g.subscribe.apply(g,[d].concat("function"===typeof c?[c]:c))}else y[E]=z.API[E];E=[["Helvetica","helvetica","normal"],["Helvetica-Bold","helvetica","bold"],["Helvetica-Oblique","helvetica","italic"],["Helvetica-BoldOblique","helvetica","bolditalic"],["Courier","courier","normal"],
...
c=e<<18|d<<12|a<<6|f,e=c>>16&255,d=c>>8&255,c&=255,64==a?s[l++]=String.fromCharCode(e):64==f?s[l++]=String.fromCharCode(e,d):s[l++]=String.fromCharCode(e,d,c);while(b<g.length);return a=s.join("")});var n="function"===typeof Object.keys?function(g){return Object.keys(g).length}:function(g){var e=0,d;for(d in g)g.hasOwnProperty(d)&&e++;return e},v=function(g){this.topics={};this.context=g;this.publish=function(e,d){if(this.topics[e]){var c=this.topics[e];d=Array.prototype.slice.call(arguments,1);var a=
...
a("/DecodeParms <<"+d.dp+">>");if("trns"in d&&d.trns.constructor==Array)for(var b="",l=0;l<d.trns.length;l++)b+=d[b][l]+" "+d.trns[l]+" ",a("/Mask ["+b+"]");"smask"in d&&a("/SMask "+(c+1)+" 0 R");a("/Length "+d.data.length+">>");f(d.data);a("endobj")}},v=function(){var g=this.internal.collections.addImage_images,e=this.internal.write,d,c;for(c in g)d=g[c],e("/I"+d.i,d.n,"0","R")};z.addImage=function(g,e,d,c,a,f){if("JPEG"!==e.toUpperCase())throw Error("addImage currently only supports format 'JPEG', not '"+
e+"'");var b;e=this.internal.collections.addImage_images;var l=this.internal.getCoordinateString,s=this.internal.getVerticalCoordinateString;"data:image/jpeg;base64,"===g.substring(0,23)&&(g=atob(g.replace("data:image/jpeg;base64,","")));if(e)if(Object.keys)b=Object.keys(e).length;else{var h=e,m=0;for(b in h)h.hasOwnProperty(b)&&m++;b=m}else b=0,this.internal.collections.addImage_images=e={},this.internal.events.subscribe("putResources",n),this.internal.events.subscribe("putXobjectDict",v);a:{var h=
g,x;if(255===!h.charCodeAt(0)||216===!h.charCodeAt(1)||255===!h.charCodeAt(2)||224===!h.charCodeAt(3)||74===!h.charCodeAt(6)||70===!h.charCodeAt(7)||73===!h.charCodeAt(8)||70===!h.charCodeAt(9)||0===!h.charCodeAt(10))throw Error("getJpegSize requires a binary jpeg file");x=256*h.charCodeAt(4)+h.charCodeAt(5);for(var m=4,u=h.length;m<u;){m+=x;if(255!==h.charCodeAt(m))throw Error("getJpegSize could not find the size of the image");if(192===h.charCodeAt(m+1)){x=256*h.charCodeAt(m+5)+h.charCodeAt(m+6);
...
n.prototype.setBlockStyle=function(a){this.paragraph.blockstyle=a};n.prototype.addText=function(a,b){this.paragraph.text.push(a);this.paragraph.style.push(b)};var e={helvetica:"helvetica","sans-serif":"helvetica",serif:"times",times:"times","times new roman":"times",monospace:"courier",courier:"courier"},d={100:"normal",200:"normal",300:"normal",400:"normal",500:"bold",600:"bold",700:"bold",800:"bold",900:"bold",normal:"normal",bold:"bold",bolder:"bold",lighter:"normal"},c={normal:"normal",italic:"italic",
oblique:"italic"},a={normal:1};z.fromHTML=function(a,b,c,d){if("string"===typeof a){var e="jsPDFhtmlText"+Date.now().toString()+(1E3*Math.random()).toFixed(0);$('<div style="position: absolute !important;clip: rect(1px 1px 1px 1px); /* IE6, IE7 */clip: rect(1px, 1px, 1px, 1px);padding:0 !important;border:0 !important;height: 1px !important;width: 1px !important; top:auto;left:-100px;overflow: hidden;"><iframe style="height:1px;width:1px" name="'+e+'" /></div>').appendTo(document.body);a=$(window.frames[e].document.body).html(a)[0]}b=
new n(this,b,c,d);g(a,b,d.elementHandlers);return b.dispose()}})(jsPDF.API);
(function(z){z.addSVG=function(n,v,g,e,d){if(void 0===v||void 0===v)throw Error("addSVG needs values for 'x' and 'y'");var c=document.createElement("iframe"),a=document.createElement("style");a.type="text/css";a.styleSheet?a.styleSheet.cssText=".jsPDF_sillysvg_iframe {display:none;position:absolute;}":a.appendChild(document.createTextNode(".jsPDF_sillysvg_iframe {display:none;position:absolute;}"));document.getElementsByTagName("head")[0].appendChild(a);c.name="childframe";c.setAttribute("width",
0);c.setAttribute("height",0);c.setAttribute("frameborder","0");c.setAttribute("scrolling","no");c.setAttribute("seamless","seamless");c.setAttribute("class","jsPDF_sillysvg_iframe");document.body.appendChild(c);c=(c.contentWindow||c.contentDocument).document;c.write(n);c.close();c=c.getElementsByTagName("svg")[0];n=[1,1];var a=parseFloat(c.getAttribute("width")),f=parseFloat(c.getAttribute("height"));a&&f&&(e&&d?n=[e/a,d/f]:e?n=[e/a,e/a]:d&&(n=[d/f,d/f]));c=c.childNodes;e=0;for(d=c.length;e<d;e++)if(a=
c[e],a.tagName&&"PATH"===a.tagName.toUpperCase()){for(var a=a.getAttribute("d").split(" "),f=parseFloat(a[1]),b=parseFloat(a[2]),l=[],s=3,h=a.length;s<h;)"c"===a[s]?(l.push([parseFloat(a[s+1]),parseFloat(a[s+2]),parseFloat(a[s+3]),parseFloat(a[s+4]),parseFloat(a[s+5]),parseFloat(a[s+6])]),s+=7):"l"===a[s]?(l.push([parseFloat(a[s+1]),parseFloat(a[s+2])]),s+=3):s+=1;a=[f,b,l];a[0]=a[0]*n[0]+v;a[1]=a[1]*n[1]+g;this.lines.call(this,a[2],a[0],a[1],n)}return this}})(jsPDF.API);
