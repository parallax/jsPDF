(function (jsPDFAPI) {
	'use strict';
		...
		var diacritics = [1612, 1613, 1614, 1615, 1616];
		...
		function getCorrectForm(currentChar, beforeChar, nextChar, arabicSubstition) {
			...
			var arabicSubst = Object.assign(arabicsubst, arabicSubstition);
			if (arabicSubst[currentChar.charCodeAt(0)] === undefined) {
				return -1;
			}
			...
			if (
					(
						(
							...
							&& (arabicSubst[beforeChar.charCodeAt(0)] !== undefined) //beforeChar is arabic
							&& (arabicSubst[beforeChar.charCodeAt(0)][initialForm] === undefined) //beforeChar has no initialForm
						) 
						||
						(
							(beforeChar === undefined)
							|| (arabicSubst[beforeChar.charCodeAt(0)] === undefined)
							|| (arrayContainsElement(endedletter, beforeChar.charCodeAt(0)))
						)
					)
					&&
						(
							(nextChar !== undefined)
							&& (arabicSubst[nextChar.charCodeAt(0)] !== undefined)
						)
				)
			{
				return initialForm;
			...
			if ((arabicSubst[currentChar.charCodeAt(0)][finalForm] === undefined)
					||
						(
							(nextChar === undefined)
							|| (arabicSubst[nextChar.charCodeAt(0)] !== undefined)
			...
			if (
					(	//current arabic letter has only final form
						arabicSubst[currentChar.charCodeAt(0)][initialForm] === undefined
					)
					||
					(
						(
						...
						(
							(beforeChar !== undefined) 
							&& (arabicSubst[beforeChar.charCodeAt(0)] !== undefined)
							&& arabicSubst[beforeChar.charCodeAt(0)][initialForm] !== undefined	
							&& arrayContainsElement(endedletter, beforeChar.charCodeAt(0)) === false
						)
					)
		...
		function processArabic(text) {
			...
			for (i = 0; i < text.length; i += 1) {
				...
				if (!isArabicLetter(currentLetter)) {
					...
					if ((prevLetter !== undefined && prevLetter.charCodeAt(0) == 1604 
						&&  arrayContainsElement(alfletter, currentLetter.charCodeAt(0)))) {
					...
					} else if ((prevLetter !== undefined && prevLetter.charCodeAt(0) == 1617 
						&&  arrayContainsElement(diacritics, currentLetter.charCodeAt(0)))) {
						localPrevLetter = text[i-2];
						localCurrentLetter = currentLetter;
						localNextLetter = text[i+1]
						position = getCorrectForm(localCurrentLetter,localPrevLetter , localNextLetter, arabicorigsubst);
						resultingLetter = String.fromCharCode(unicode_diacritics[currentLetter.charCodeAt(0)][position]);
						result = result.substr(0, result.length - 1) + resultingLetter;
					} else {
