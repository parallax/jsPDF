    ...
    stroke: function() {
      ...
      for (var i = 0; i < this.path.length; i++) {
        ...
        switch (pt.type) {
          ...
          case "bct":
            var delta = [
              pt.x1 - this.path[i - 1].x,
              pt.y1 - this.path[i - 1].y,
              pt.x2 - this.path[i - 1].x,
              pt.y2 - this.path[i - 1].y,
              pt.x - this.path[i - 1].x,
              pt.y - this.path[i - 1].y
            ];
            deltas.push(delta);
            break;
          case "qct":
            // convert to bezier
            var x1 =
              this.path[i - 1].x + (2.0 / 3.0) * (pt.x1 - this.path[i - 1].x);
            var y1 =
              this.path[i - 1].y + (2.0 / 3.0) * (pt.y1 - this.path[i - 1].y);
