...
(function(jsPDFAPI) {
   ...
   * @ignore
   */
  let createElement = function(tagName, opt) {
   ...
   * @ignore
   */
  let cloneNode = function(node, javascriptEnabled) {
  ...
  let Worker = function Worker(opt) {
    // Create the root parent for the proto chain, and the starting Worker.
    let root = Object.assign(
      Worker.convert(Promise.resolve()),
      JSON.parse(JSON.stringify(Worker.template))
    );
    let self = Worker.convert(Promise.resolve(), root);
  ...
  Worker.prototype.toCanvas = function toCanvas() {
    ...
    return this.thenList(prereqs)
      ...
      .then(function toCanvas_main(html2canvas) {
        ...
        let options = Object.assign({}, this.opt.html2canvas);
      ...
      .then(function toCanvas_post(canvas) {
        // Handle old-fashioned 'onrendered' argument.
        let onRendered = this.opt.html2canvas.onrendered || function() {};
        onRendered(canvas);
  ...
  Worker.prototype.toContext2d = function toContext2d() {
    ...
    return this.thenList(prereqs)
      ...
      .then(function toContext2d_main(html2canvas) {
        ...
        let pdf = this.opt.jsPDF;
        let fontFaces = this.opt.fontFaces;
        ...
        if (fontFaces) {
          for (let i = 0; i < fontFaces.length; ++i) {
            let font = fontFaces[i];
            let src = font.src.find(function(src) {
  ...
  Worker.prototype.toImg = function toImg() {
    ...
    return this.thenList(prereqs).then(function toImg_main() {
      let imgData = this.prop.canvas.toDataURL(
  ...
  Worker.prototype.set = function set(opt) {
    ...
    let fns = Object.keys(opt || {}).map(function(key) {
  ...
  Worker.prototype.get = function get(key, cbk) {
    return this.then(function get_main() {
      ...
      let val = key in Worker.template.prop ? this.prop[key] : this.opt[key];
  ...
  Worker.prototype.thenCore = function thenCore(
    ...
    let selfPromise = isNative
      ? self
      : Worker.convert(Object.assign({}, self), Promise.prototype);

    // Return the promise, after casting it into a Worker and preserving props.
    let returnVal = thenBase.call(selfPromise, onFulfilled, onRejected);
  ...
  Worker.prototype["catch"] = function(onRejected) {
    ...
    let returnVal = Promise.prototype["catch"].call(this, onRejected);
